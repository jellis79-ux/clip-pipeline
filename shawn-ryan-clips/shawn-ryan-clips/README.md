...
5. **Add repo secrets** (Settings → Secrets and variables → Actions → New
   ...
   - `ANTHROPIC_API_KEY`
   - `AYRSHARE_API_KEY`
   - `TARGET_PLATFORMS` — start with `youtube` only for testing (see
...
## Testing phase: start with YouTube only

Before posting everywhere, set `TARGET_PLATFORMS=youtube` and run
against a single new handle. Reasons to start here specifically:

- **Lowest cost of a mistake** — a bad clip pick or awkward commentary
  card is far less exposed than on TikTok/Instagram, and easy to quietly
  delete or fix.
- **Clearest monetization path** — YouTube's Partner Program threshold
  (500 subs + 3M Shorts views or 3,000 watch hours) is well-defined, and
  Shorts have lasting search value, unlike TikTok/Instagram which reward
  posting frequency more than any single clip's quality.
...
few real posts), widen `TARGET_PLATFORMS` to include Instagram, TikTok,
and X.
